...
/* json */
import lineJson from "@/assets/json/fmjd/fmjd.json";
import gridJson from "@/assets/json/fmjd/line.json";
import polygonJson from "@/assets/json/fmjd/gridMain.json";

/* tool */
import { wgs84togcj02 } from "@/utils/transform.js";

import { CesiumObj } from "@/utils/Object/cesium_config.js";
...
const models = {
  hnc: { name: "环南村", height: 18 },
  hqc: { name: "红旗村", height: 18 },
  hxc: { name: "合星村", height: 18 },
  jsc: { name: "建胜村", height: 18 },
  lac1: { name: "灵安村1", height: 18 },
  lac2: { name: "灵安村2", height: 18 },
  lagyy1: { name: "灵安工业园1", height: 18 },
  lagyy2: { name: "灵安工业园2", height: 18 },
  lasq: { name: "灵安社区", height: 18 },
  ljyc: { name: "路家园村", height: 18 },
  lzc: { name: "联庄村", height: 18 },
  tfgyy: { name: "同福工业园", height: 18 },
  xncc: { name: "新农村村", height: 18 },
  xnqc: { name: "西牛桥村", height: 18 },
  zqc: { name: "中群村", height: 18 },
};
const id = "fmjd";
const cesiumObj = new CesiumObj(models, polygonJson);
...
let center = {
  ...
  lat: 30.56656,
  height: 184.69074,
  heading: 0.24100963528487007,
  pitch: -0.47487493807627956,
  roll: 0.000789289363456902,
};
...
class Fmjd {
  ...
  // models = {
  //   // 同福工业区new
  //   TFGYY: { name: '同福工业区new', height: 18 },
  //   'TFD-YS': { name: '同福集镇东，同福东', height: 10 },
  //   'HJB-YS': { name: '胡家浜', height: 23 },
  //   CJB: { name: '崔加滨,崔家滨', height: 10 },
  //   'FJDQ-YS': { name: '冯家大桥,冯家大桥', height: 17 },
  //   'FYDN-YS': { name: '福严东南,福严东南', height: 10 },
  //   'TFGYYB-YS': { name: '同福工业区北,同福工业园北', height: 10 },
  //   XJD: { name: '夏家兜,夏家兜', height: 0 },
  //   'TFX-YS': { name: '同福集镇西, 同福西', height: 6 },
  //   'FYS-YCM-YS': { name: '福严禅寺+永昌庙', height: 7 },
  //   'TFJZ-YS': { name: '同福集镇, 同福集镇', height: 10 },
  //   'HN-HNX-LA-YS': { name: '环南+环南西+灵安', height: 17 }
  // }
  models = {
    hnc: { name: "环南村", height: 18 },
    hqc: { name: "红旗村", height: 18 },
  ...
  init() {
    ...
    this.viewer = new Cesium.Viewer("fmjd", {
    ...
    this.addLine(lineJson, "red", "行政区划线", true, "lineArr").then((dataJson) =>
      this.addWall(dataJson)
    );
    /* 这边必须深拷贝,否则会影响切换时数据的改变,因为后续会改变里面的经纬度偏移 */
    this.addLine(
      JSON.parse(JSON.stringify(gridJson)),
      "yellow",
      "行政网格线",
      false,
      "gridArr"
    );
    // .then(dataJson => this.addWall(dataJson))
  ...
  rotateEarth() {
    const onTickCallback = () => {
      const spinRate = 0.5;
      const currentTime = that.viewer.clock.currentTime.secondsOfDay;
  ...
  loadLayer() {
    ...
    // 设置地球球体颜色
    // this.viewer.scene.globe.baseColor = Cesium.Color.fromBytes(92, 158, 92,.1); 
  ...
  loadModel() {
    ...
    */
    ...
    this.changeCenter(center, "drawLineAndWall");
  }
  // 设置摄像机
  setCamera() {
    // 形状大小
  ...
  drawLineAndWall() {
    ...
    const addPolygon = () => {
      ...
      const promise = Cesium.GeoJsonDataSource.load(polygonJson);
      promise.then((data) => {
  ...
  // 添加边界线
  addLine(json, color, type, bool, arr) {
    ...
    let dataJson = arr === "gridArr" ? json : polygon2line(json);
    ...
    promise.then((data) => {
      ...
      this.viewer.dataSources.add(data);
      this[arr] = data.entities.values;
      // const villageName = ["西牛桥村", "路家园村", "建胜村", "合星村", "环南村", "灵安村", "灵安社区", "灵安工业园", "中群村", "新农村村", "联庄村", "同福工业园", "红旗村", "灵安园区"]
      this[arr].forEach((entity) => {
  ...
  */
  ...
  pointPicked() {
    ...
    */
    ...
    handler.setInputAction((click) => {
      ...
      console.log('heading=', this.viewer.scene.camera.heading)
      console.log('pitch=', this.viewer.scene.camera.pitch)
      console.log('roll=', this.viewer.scene.camera.roll) 
      console.log(cameraLon, cameraLat, cameraHeight, "cameraLon & cameraLat & cameraHeight");
  ...
  drawShape(type) {
    ...
    this.handler.setInputAction((movement) => {
      ...
      if (Cesium.defined(earthPosition)) {
        this.pointsArr.push(createPoint(earthPosition));
  ...
  // 画圆
  drawCircle() {
    let circle_center_entity = null; // 圆心点 entity
    let circle_temporary_entity = null; // 临时圆形entity
    let circle_entity = null; // 结果圆形entity
    let circle_end_point = null; // 结束点
    let circle_center_point = null; // 圆心点

    if (circle_entity !== null) {
      this.viewer.remove(circle_temporary_entity);
      this.viewer.remove(circle_entity);
      circle_center_entity = null;
    }

    let circle = { entity: null, r: 0, center: null, type: "circle" };
    const callBackPos = (arr) => {
      circle.r = Math.sqrt(
        Math.pow(arr[0].x - arr[arr.length - 1].x, 2) +
          Math.pow(arr[0].y - arr[arr.length - 1].y, 2)
      );
      return circle.r ? circle.r : circle.r + 1;
    };
    shape = this.viewer.entities.add({
      position: this.pointsArr[0],
      ellipse: {
        //半径 两点间距离
    ...
    circle.entity = shape;
    circle.center = this.callBack([{ ...arr[0] }]);
  }
  // 画矩形
  drawRectangle() {
    let rectangle = { entity: null, points: null, type: "rectangle" };
  }
  // 画多边形
  drawPolygon() {
    let polygon = { entity: null, points: null, type: "polygon" };
  }
}
const fmjd = new Fmjd()
onMounted(() => {
  ...
  // cesiumObj
  //   .addLine(lineJson, "red", "行政区划线", true, "面")
  //   .then((dataJson) => cesiumObj.addWall(dataJson, "rgba(55,96,56,0.7)"));
  /* 如果需要从 面格式数据转换成线格式数据,必须深拷贝,否则会影响切换时数据的改变,因为后续会改变里面的经纬度偏移 */
  // cesiumObj
  //   .addLine(JSON.parse(JSON.stringify(gridJson)), "yellow", "行政网格线", true, "面")
  //   .then((dataJson) => cesiumObj.addWall(dataJson, "rgba(255,0,0,0.3)"));

  // 改变中心
  cesiumObj.changeCenter(center, "drawLineAndWall");
});
</script>
